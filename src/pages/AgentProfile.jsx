...
export default function AgentProfile() {
  ...
  return (
    <main className="min-h-screen font-sans bg-white dark:bg-carbon-800">
      <Navbar />

      {/* Responsive Hero Banner */}
      {agent.banner && (
        <section className="relative w-full bg-white dark:bg-carbon-800 overflow-hidden">
          ...
          <>
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left text-gray-600 max-w-2xl mx-auto mb-10">
              {agent.capabilities.map((cap, idx) => (
                <li
                  key={idx}
                  className={`bg-white dark:bg-carbon-800 ${agent.themeColor?.border || "border-gray-200"} px-4 py-3 rounded-lg shadow-sm hover:shadow-md transition`}
                >
                  ✅ {cap}
                </li>
              ))}
